...
class DataLoader:
    ...
    def basic_plot(self,method): #画近期行情
        ...
        if method=='performance':
            ...
            date = pd.to_datetime('2023')
            data_2023 = data.loc[date]
            data_2023['close'].plot(label='close')
            data_2023['MA5'].plot(label='MA5')
            data_2023['MA10'].plot(label='MA10')
            data_2023['MA120'].plot(label='MA120')
            plt.legend()
            plt.subplot(212)
            plt.title('000001SH Volatility Week in 2023')
            y=data_2023['return'].resample('W').std()
